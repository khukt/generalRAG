...
def generate_text(model, tokenizer, task_type, question, context, max_length, num_beams, no_repeat_ngram_size, early_stopping, use_template, templates, question_type):
    try:
        ...
        return format_output(answer), memory_footprint, attentions, input_text
...
if question:
    ...
    # Concatenate context, question, and task type for tokenization and attention alignment
    input_text = f"{context} {question} {task_type}"
    
    with st.spinner("Generating..."):
        guide, memory_footprint, attentions, input_text = generate_text(model, tokenizer, task_type, question, context, max_length, num_beams, no_repeat_ngram_size, early_stopping, use_template, template_manager.get_templates(), question_type)
